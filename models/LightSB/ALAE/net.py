...
class EncoderDefault(nn.Module):
    def __init__(self, startf, maxf, layer_count, latent_size, channels=3):
        ...
        for i in range(self.layer_count):
            ...
            block = EncodeBlock(
                inputs, outputs, latent_size, False, fused_scale=fused_scale
            )
...
class Generator(nn.Module):
    def __init__(self, startf=32, maxf=256, layer_count=3, latent_size=128, channels=3):
        ...
        for i in range(self.layer_count):
            ...
            self.style_sizes += [
                2 * (inputs if has_first_conv else outputs),
                2 * outputs,
            ]
...
class MappingD(nn.Module):
    def __init__(
        self, mapping_layers=5, latent_size=256, dlatent_size=256, mapping_fmaps=256
    ):
