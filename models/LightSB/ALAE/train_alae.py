...
def save_sample(
    ...
    with torch.no_grad():
        ...
        if lod2batch.in_transition:
            needed_resolution_prev = model.decoder.layer_to_resolution[
                lod2batch.lod - 1
            ]
            sample_in_prev = F.avg_pool2d(sample_in, 2, 2)
            sample_in_prev_2x = F.interpolate(sample_in_prev, needed_resolution)
            sample_in = sample_in * blend_factor + sample_in_prev_2x * (
                1.0 - blend_factor
            )
        ...
        def save_pic(x_rec):
            tracker.register_means(
                ...
                + lod2batch.iteration * 1.0 / lod2batch.get_dataset_size()
            )
            tracker.plot()
            ...
            f = os.path.join(
                cfg.OUTPUT_DIR,
                "sample_%d_%d.jpg"
                % (lod2batch.current_epoch + 1, lod2batch.iteration // 1000),
            )
            print("Saved to %s" % f)
            save_image(
                result_sample, f, nrow=min(32, lod2batch.get_per_GPU_batch_size())
            )
...
def train(cfg, logger, local_rank, world_size, distributed):
    ...
    decoder_optimizer = LREQAdam(
        [{"params": decoder.parameters()}, {"params": mapping_f.parameters()}],
    ...
    lod2batch = lod_driver.LODDriver(
        cfg, logger, world_size, dataset_size=len(dataset) * world_size
    )
    ...
    for epoch in range(scheduler.start_epoch(), cfg.TRAIN.TRAIN_EPOCHS):
        ...
        batches = make_dataloader(
            cfg, logger, dataset, lod2batch.get_per_GPU_batch_size(), local_rank
        )
