...
class LODDriver:
    ...
    def get_blend_factor(self):
        if self.cfg.TRAIN.EPOCHS_PER_LOD == 0:
            return 1
        blend_factor = float(
            (self.current_epoch % self.cfg.TRAIN.EPOCHS_PER_LOD) * self.dataset_size
            + self.iteration
        )
    ...
    def is_time_to_report(self):
        if (
            self.iteration
            >= self.tick_start_nimg_report
            + self.reports[min(self.lod, len(self.reports) - 1)] * 1000
        ):
    ...
    def is_time_to_save(self):
        if (
            self.iteration
            >= self.tick_start_nimg_snapshot
            + self.snapshots[min(self.lod, len(self.snapshots) - 1)] * 1000
        ):
    ...
    def set_epoch(self, epoch, optimizers):
        ...
        new_lod = min(
            self.cfg.MODEL.LAYER_COUNT - 1, epoch // self.cfg.TRAIN.EPOCHS_PER_LOD
        )
        if new_lod != self.lod:
            self.lod = new_lod
            self.logger.info("#" * 80)
        ...
        is_in_first_half_of_cycle = (epoch % self.cfg.TRAIN.EPOCHS_PER_LOD) < (
            self.cfg.TRAIN.EPOCHS_PER_LOD // 2
        )
