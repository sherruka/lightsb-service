...
class Model(nn.Module):
    ...
    def forward(self, x, lod, blend_factor, d_train, ae):
        if ae:
            ...
            s, rec = self.generate(
                lod, blend_factor, z=z, mixing=False, noise=True, return_styles=True
            )
            ...
            rec = self.generate(
                lod, blend_factor, count=x.shape[0], z=z.detach(), noise=True
            )
    ...
    def lerp(self, other, betta):
        ...
        with torch.no_grad():
            params = (
                list(self.mapping_d.parameters())
                + list(self.mapping_f.parameters())
                + list(self.decoder.parameters())
                + list(self.encoder.parameters())
                + list(self.dlatent_avg.parameters())
            )
            other_param = (
                list(other.mapping_d.parameters())
                + list(other.mapping_f.parameters())
                + list(other.decoder.parameters())
                + list(other.encoder.parameters())
                + list(other.dlatent_avg.parameters())
            )
