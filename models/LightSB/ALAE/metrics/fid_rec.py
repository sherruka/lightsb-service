    ...
    def evaluate(self, logger, mapping, decoder, encoder, lod):
        ...
        def compute_for_reals(num_images, path, lod):
            ...
            batches = make_dataloader(
                self.cfg, logger, dataset, self.minibatch_size, 0, numpy=True
            )
        ...
        mu_real, sigma_real = compute_for_reals(
            self.num_images, self.cfg.DATASET.PATH, lod
        )
        ...
        for idx, x in tqdm(enumerate(batches)):
            ...
            images = np.clip((images.cpu().numpy() + 1.0) * 127, 0, 255).astype(
                np.uint8
            )
...
def sample(cfg, logger):
    ...
    last_epoch = list(extra_checkpoint_data["auxiliary"]["scheduler"].values())[0][
        "last_epoch"
    ]
    ...
    with torch.no_grad():
        ...
        ppl.evaluate(
            logger, mapping_fl, decoder, encoder, cfg.DATASET.MAX_RESOLUTION_LEVEL - 2
        )
...
if __name__ == "__main__":
    ...
    run(
        sample,
        get_cfg_defaults(),
        description="ALAE-fid-reconstruction",
        default_config="configs/ffhq.yaml",
        world_size=gpu_count,
        write_log="metrics/fid_score-reconstruction.txt",
    )
