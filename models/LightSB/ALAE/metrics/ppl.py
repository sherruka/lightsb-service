...
class PPL:
    def __init__(
        self, cfg, num_samples, epsilon, space, sampling, minibatch_size, **kwargs
    ):
        assert space in ["z", "w"]
    ...
    def evaluate(self, logger, mapping, decoder, lod, celeba_style=False):
        ...
        for _ in tqdm.tqdm(range(0, self.num_samples, self.minibatch_size)):
            ...
            lat_t01 = torch.randn(
                [self.minibatch_size * 2, self.cfg.MODEL.LATENT_SPACE_SIZE]
            )
            lerp_t = torch.rand(self.minibatch_size) * (
            ...
            if self.space == "w":
                ...
                dlat_e1 = torch.lerp(
                    dlat_t0, dlat_t1, lerp_t[:, np.newaxis, np.newaxis] + self.epsilon
                )
                dlat_e01 = torch.reshape(
                    torch.stack([dlat_e0, dlat_e1], dim=1), dlat_t01.shape
                )
            else:  # space == 'z'
                lat_t0, lat_t1 = lat_t01[0::2], lat_t01[1::2]
                lat_e0 = slerp(lat_t0, lat_t1, lerp_t[:, np.newaxis])
                lat_e1 = slerp(lat_t0, lat_t1, lerp_t[:, np.newaxis] + self.epsilon)
                lat_e01 = torch.reshape(
                    torch.stack([lat_e0, lat_e1], dim=1), lat_t01.shape
                )
