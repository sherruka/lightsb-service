...
def parse_tfrecord_np(record):
    ...
    return (
        np.fromstring(data, np.uint8).reshape(shape),
        np.fromstring(dlat, np.float32),
        np.fromstring(lat, np.float32),
    )
...
class Predictions:
    ...
    def evaluate(self, logger, mapping, decoder, lod, attrib_idx):
            ...
            while True:
                try:
                    ...
                    logits = classifier.run(
                        images, None, num_gpus=1, assume_frozen=True
                    )
                    logits = torch.tensor(logits)
                    predictions = torch.softmax(
                        torch.cat([logits, -logits], dim=1), dim=1
                    )
        ...
        results = {
            key: np.concatenate([value[key] for value in result_expr], axis=0)
            for key in result_expr[0].keys()
        }
...
def main(cfg, logger):
    ...
    with torch.no_grad():
        ...
        for i in indices:
            p.evaluate(
                logger, mapping_fl, decoder, cfg.DATASET.MAX_RESOLUTION_LEVEL - 2, i
            )
