...
class TFRecordsDataset:
    def __init__(
        ...
        for r in range(2, cfg.DATASET.MAX_RESOLUTION_LEVEL + 1):
            files = []
            for i in range(
                self.part_count_local * rank, self.part_count_local * (rank + 1)
            ):
                file = path % (r, i)
                files.append(file)
    ...
    def reset(self, lod, batch_size):
        ...
        if self.needs_labels:
            self.features = {
                ...
                "data": db.FixedLenFeature(
                    [self.channels, img_size, img_size], db.uint8
                ),
...
def make_dataloader(cfg, logger, dataset, GPU_batch_size, local_rank, numpy=False):
    ...
    batches = db.data_loader(
        iter(dataset), BatchCollator(local_rank), len(dataset) // GPU_batch_size
    )
...
def make_imagenet_dataloader(
    cfg, logger, dataset, GPU_batch_size, target_size, local_rank, do_random_crops=True
):
    class BatchCollator(object):
        def __init__(self, device=torch.device("cpu")):
            self.device = device
...
def make_imagenet_dataloader_y(
    cfg, logger, dataset, GPU_batch_size, target_size, local_rank, do_random_crops=True
):
    class BatchCollator(object):
        def __init__(self, device=torch.device("cpu")):
