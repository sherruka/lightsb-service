...
class LightSB(nn.Module):
    def __init__(
        ...
        self.log_alpha_raw = nn.Parameter(
            self.epsilon * torch.log(torch.ones(n_potentials) / n_potentials)
        )
    ...
    def get_S(self):
        if self.is_diagonal:
            ...
            S = (
                self.S_rotation_matrix
                * (torch.exp(self.S_log_diagonal_matrix))[:, None, :]
            ) @ torch.permute(self.S_rotation_matrix, (0, 2, 1))
        return S
    ...
    def forward(self, x):
        ...
        for i in range(num_sampling_iterations):
            ...
            if self.is_diagonal:
                x_S_x = (
                    sub_batch_x[:, None, :] * S[None, :, :] * sub_batch_x[:, None, :]
                ).sum(dim=-1)
                x_r = (sub_batch_x[:, None, :] * r[None, :, :]).sum(dim=-1)
                r_x = r[None, :, :] + S[None, :] * sub_batch_x[:, None, :]
            else:
                x_S_x = (
                    sub_batch_x[:, None, None, :]
                    @ (S[None, :, :, :] @ sub_batch_x[:, None, :, None])
                )[:, :, 0, 0]
                x_r = (sub_batch_x[:, None, :] * r[None, :, :]).sum(dim=-1)
                r_x = (
                    r[None, :, :]
                    + (S[None, :, :, :] @ sub_batch_x[:, None, :, None])[:, :, :, 0]
                )
    ...
    def get_drift(self, x, t):
        ...
        A_diagonal = (t / (epsilon * (1 - t)))[:, None, None] + 1 / (
            epsilon * S_diagonal
        )[
            None, :, :
        ...
        if self.is_diagonal:
            ...
            S = (self.S_rotation_matrix * S_diagonal[:, None, :]) @ torch.permute(
                self.S_rotation_matrix, (0, 2, 1)
            )
            A = (
                self.S_rotation_matrix[None, :, :, :] * A_diagonal[:, :, None, :]
            ) @ torch.permute(self.S_rotation_matrix, (0, 2, 1))[None, :, :, :]

            S_inv = (
                self.S_rotation_matrix * (1 / S_diagonal[:, None, :])
            ) @ torch.permute(self.S_rotation_matrix, (0, 2, 1))
            A_inv = (
                self.S_rotation_matrix[None, :, :, :] * (1 / A_diagonal[:, :, None, :])
            ) @ torch.permute(self.S_rotation_matrix, (0, 2, 1))[None, :, :, :]
        ...
        drift = (
            ...
            + epsilon
            * torch.autograd.grad(
                lse, x, grad_outputs=torch.ones_like(lse, device=lse.device)
            )[0]
        ).detach()
    ...
    def sample_at_time_moment(self, x, t):
        ...
        return (
            t * y
            + (1 - t) * x
            + torch.sqrt(t * (1 - t) * self.epsilon) * torch.randn_like(x)
        )
    ...
    def get_log_C(self, x):
        ...
        if self.is_diagonal:
            ...
            x_S_x = (x[:, None, None, :] @ (S[None, :, :, :] @ x[:, None, :, None]))[
                :, :, 0, 0
            ]
            x_r = (x[:, None, :] * r[None, :, :]).sum(dim=-1)
