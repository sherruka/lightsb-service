...
def plot_mapping(
    independent_mapping, true_mapping, predicted_mapping, target_data, n_plot, step
):
    s = 30
    linewidth = 0.2
    map_alpha = 1
    ...
    for i, mapping in enumerate(
        [independent_mapping_pca, true_mapping_pca, predicted_mapping_pca]
    ):
        inp = mapping[:, :2]
        out = mapping[:, 2:]
        ...
        axes[i].scatter(
            ...
            out[:, 1],
            s=s,
            label=mapped_data_label,
            alpha=data_alpha,
            zorder=2,
            color=mapped_data_color,
        )
...
def plot_2D_trajectory(
    ...
    trajectory,
    step,
    lims=((-2.25, 2.5), (-2.25 / 1.5, 2.5 / 1.5)),
    x_scatter_alpha=1,
):
    fig, axes = plt.subplots(
        1, 1, figsize=(6, 4), squeeze=True, sharex=True, sharey=True
    )
...
def plot_2D_mapping(
    ...
    independent_mapping_pca = np.concatenate(
        (independent_mapping[:n_plot, :dim], independent_mapping[:n_plot, dim:]),
        axis=-1,
    )
    predicted_mapping_pca = np.concatenate(
    ...
    fig, axes = plt.subplots(
        1, 2, figsize=(8, 4), squeeze=True, sharex=True, sharey=True
    )
